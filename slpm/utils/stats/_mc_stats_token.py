...
from ._mpi_stats_token import mean as _mean
from ._mpi_stats_token import var as _var
from ._mpi_stats_token import total_size as _total_size
...
class Stats:
    ...
    def __getattr__(self, name):
        if name in ("mean", "Mean"):
            ...
            raise AttributeError("'Stats' object object has no attribute '{}'".format(name))
...
def _block_variance(data, l, *, token=None, comm=MPI_jax_comm):
    blocks = _get_blocks(data, l)
    ts = _total_size(blocks)
    if ts > 0:
        res, token = _var(blocks, token=token, comm=comm)
        return res, ts, token
    else:
        return jnp.nan, 0, token


def _batch_variance(data, *, token=None, comm=MPI_jax_comm):
    b_means = data.mean(axis=1)
    ts = _total_size(b_means)
    res, token = _var(b_means, token=token, comm=MPI_jax_comm)
    return res, ts, token


# this is not batch_size maybe?
def statistics(data, batch_size=32, *, token=None):
    r"""
    like nk.stats.statistics but with support for mpi4jax tokens
    ...
    """
    return _statistics(data, batch_size, token=token)


# TODO figure out a way to thread the MPI communicator throught jit
# (after all it should just be a pointer, right?)
...
def _statistics(data, batch_size, *, token=None):
    ...
    def batch_good_err(args):
        ...
        return jnp.asarray(error_of_mean, dtype=stat_dtype), jnp.asarray(tau_corr, dtype=stat_dtype)
    ...
    def nan_err(args):
        return jnp.asarray(jnp.nan, dtype=stat_dtype), jnp.asarray(jnp.nan, dtype=stat_dtype)
    ...
    if n_batches > 1:
        ...
        if not config.FLAGS["NETKET_USE_PLAIN_RHAT"]:
            ...
            if N % 2 == 0:
                ...
                batch_var, _, token = _batch_variance(data.reshape(2 * local_batch_size, N // 2), token=token)
            else:
                # drop the last sample of each chain for an even split,
                # like [[1 2 3 4 5]] -> [[1 2][3 4]]
                batch_var, _, token = _batch_variance(data[:, :-1].reshape(2 * local_batch_size, N // 2), token=token)

        # V_loc = _np.var(data, axis=-1, ddof=0)
        # W_loc = _np.mean(V_loc)
        # W, token = _mean(W_loc, token=token, comm=comm)
